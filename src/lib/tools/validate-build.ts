  ...
  return tool({
    ...
    execute: async ({ label, parts }: { label?: string; parts: BuildParts }) => {
      ...
      if (!ids.length) {
        ...
        const snapshot = createBuildSnapshot({ label, parts, validation, productsById: new Map(), scope });
      ...
      const snapshot = createBuildSnapshot({ label, parts, validation, productsById: byId, scope });
      return { ...validation, snapshot };
    }
  });
}
